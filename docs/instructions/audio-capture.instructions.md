...
- `/settings/audio` owns the dedicated Audio settings route. It may show browser audio-processing toggles, current recording interaction mode, an explicit user-triggered input test, and reset calibration, while actual `getSettings()` values, sample rate, noise/level metrics, calibration readings, and AudioWorklet state stay inside `Advanced audio diagnostics`.
- Enrollment prompt recording UI must keep capture start/stop behind explicit user gestures, present only one dominant record/stop control for the current prompt, map recording-quality results to concise actionable copy, and put setup, SNR/clipping/VAD/alignment/pace/confidence metrics, stored-recording summaries, and replay/retry/accept/skip controls in state-relevant secondary actions or `Recording details` without changing cleanup semantics.
