...
- Bilingual cohort release reports may include only aggregate per-speaker quality rates, public slice labels, counts, medians/ratios, and gate decisions. Synthetic fixtures validate schema/math only and must never be cited as production quality evidence.
- v0.5.0 personal-model release benchmark reports belong in `@speech/benchmark` and may include only aggregate training duration, peak browser memory, peak additional storage, adapter RTF overhead, profile-swap latency, export/import duration, checkpoint loss delta, zero-network count, offline reload result, and declared reference-hardware labels. Treat synthetic/CI smoke diagnostics as insufficient release evidence.
- v0.6.0 Dictate performance parity reports also belong in `@speech/benchmark`. They may include aggregate initial JS/CSS bytes, interaction readiness, route transition, long-task count/max, CLS, recording UI response, first-partial/stable-word/finalization latency fields, and gate decisions only. Browser-smoke/fake-microphone evidence validates instrumentation but remains insufficient for release parity when v0.5 reference baselines or real-ASR reference-hardware latency data are missing.
