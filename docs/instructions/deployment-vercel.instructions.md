...
- Preserve `Permissions-Policy: microphone=(self), camera=(), geolocation=()` and a restrictive CSP. `connect-src` may include only `'self'` plus exact external origins required by committed installable model manifests; do not add wildcard hosts, broad `https:`, telemetry origins, or unreviewed CDNs.
