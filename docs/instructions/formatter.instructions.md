...
- Keep formatter options bilingual-ready with language mode/span metadata even when the v0.1.0 model emits Vietnamese-only text; language diagnostics must never force English casing over Vietnamese spans.
- Canonical vocabulary display replacement must be token-span based, explicit, deterministic, and preserve the entry `displayForm` instead of applying broad casing/title rules.
