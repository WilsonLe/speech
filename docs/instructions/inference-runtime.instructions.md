...
- Real Dictate inference belongs in the ASR worker: load encoder/predictor/joiner ONNX sessions there, resample captured PCM to the manifest sample rate, run log-Mel extraction and greedy RNN-T decoding there, and emit `FINAL` text back to the UI without main-thread model execution.
...
- Browser-training worker code may convert metadata-only ORT Training spike reports into a `BrowserTrainingBackend` selection/fallback warning, but must not import `onnxruntime-web/training`, fetch training WASM, construct fake `TrainingSession` objects, or treat fallback diagnostics as activation evidence.
...
- ASR workers own runtime-priority broadcasts for runtime initialization, profile loading, and utterance/audio activity. Priority events must be same-origin/local, redacted (no utterance IDs, audio, transcripts, profile IDs, checkpoints, or adapter weights), and used by browser-training workers to pause cooperatively and release resources when ASR becomes active.
