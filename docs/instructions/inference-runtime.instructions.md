...
- As of pinned `onnxruntime-web@1.27.0`, browser training is not exposed through a public `TrainingSession`/training subpath; keep the spike as a metadata/API capability report and do not add fake training execution. The shipping decision is deferred by `docs/adr/0001-defer-browser-training-path.md`.
