...
- Profile storage is separate from model storage: keep accepted-take WAV/metadata, export packages, browser-training private checkpoints, and active/previous profile pointers under `@speech/profile-manager` paths with safe segments, checksum verification, atomic writes, explicit import/export, and deterministic delete-all behavior. Synthetic browser-training UI demos may use browser-local storage for reload recovery, but private checkpoints must not use the model-file backend.
