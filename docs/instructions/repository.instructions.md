...
- Keep durable project docs in `docs/instructions/*.instructions.md`; keep accepted architecture decision records in `docs/adr/*.md` using the ADR template instruction. Research evidence summaries for release studies may live in `docs/research/*.md` when paired with structured, static-tested JSON artifacts and kept free of private participant data.
