...
- Personal Models compact rows, row menus, cards, `/models/new` create-model drafts, detail accordions, reason-code messages, enrollment prompt screens, and navigation may show only aggregate profile/vocabulary/speech-model/enrollment status plus explicit lifecycle controls; keep raw profile/model/utterance identifiers, hashes, storage paths, checksums, hidden prompt metadata, vocabulary terms, audio, features, checkpoints, adapter weights, and full profile JSON out of rendered summaries. `/models/new` must remain a UI-only wizard that asks one question per screen and does not create profile-store domain data until the existing enrollment/profile-store path saves an accepted take. Enrollment prompt UI should expose current prompt text only for the active recording task, keep one dominant recording action, use safe Loud guidance, keep replay/retry/accept/skip state-relevant, and place derived metrics/stored-recording summaries under `Recording details`. Export, deactivate, rollback, duplicate, and delete actions must stay worker-owned and confirmation-backed when they affect sensitive or destructive local voice data. Incompatibilities, required quality checks, recovery, and primary actions stay visible above accordions so sensitive next steps are never hidden only in collapsed technical detail.
