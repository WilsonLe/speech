...
- Personal Models compact rows, row menus, cards, `/models/new` create-model drafts, detail accordions, reason-code messages, and navigation may show only aggregate profile/vocabulary/speech-model status plus explicit lifecycle controls; keep raw profile/model identifiers, hashes, storage paths, prompts, vocabulary terms, audio, features, checkpoints, adapter weights, and full profile JSON out of rendered summaries. `/models/new` must remain a UI-only wizard that asks one question per screen and does not create profile-store domain data until the existing enrollment/profile-store path saves an accepted take. Export, deactivate, rollback, duplicate, and delete actions must stay worker-owned and confirmation-backed when they affect sensitive or destructive local voice data. Incompatibilities, required quality checks, recovery, and primary actions stay visible above accordions so sensitive next steps are never hidden only in collapsed technical detail.
