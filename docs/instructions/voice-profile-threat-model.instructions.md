...
- If personal/anchor end-to-end evaluation is added, does it compare generic/P1/candidate configurations with aggregate-only personal-heldout, anchor, language, voice-condition, custom-term, latency, RTF, and size metrics while omitting raw case IDs, prompt/reference text, selected-vocabulary entry IDs, audio, feature tensors, checkpoints, adapter weights, and profile JSON?
