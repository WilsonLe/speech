...
- If personal/anchor end-to-end evaluation, candidate-result UI, or activation review is added, does it compare generic/P1/candidate configurations with aggregate-only personal-heldout, anchor, language, voice-condition, custom-term, latency, RTF, and size metrics while omitting raw case IDs, prompt/reference text, selected-vocabulary entry IDs, hashes, storage paths, audio, feature tensors, checkpoints, adapter weights, raw profile IDs, and profile JSON? Are hard checks non-overridable, advisory checks clearly labeled, exact metrics disclosed only in grouped Results sections, and rollback/generic fallback still visible?
