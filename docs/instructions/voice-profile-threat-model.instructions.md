...
- Personal Models compact rows, row menus, cards, detail accordions, and navigation may show only aggregate profile/vocabulary/base-model status plus explicit lifecycle controls; keep raw profile/model identifiers, hashes, storage paths, prompts, vocabulary terms, audio, features, checkpoints, adapter weights, and full profile JSON out of rendered summaries. Export, deactivate, rollback, duplicate, and delete actions must stay worker-owned and confirmation-backed when they affect sensitive or destructive local voice data. Incompatibilities, hard gates, recovery, and primary actions stay visible above accordions so sensitive next steps are never hidden only in collapsed technical detail.
