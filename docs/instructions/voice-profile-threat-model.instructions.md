...
| Profile package tampering            | Verify schema, safe paths, size, checksum, top-level vs embedded metadata consistency, and base-model identity before import/enable.                                              |
...
- If a future trainer is added, does it split by prompt identity, keep held-out prompts separate, validate exported profile checksums/base-model identity before reading audio, and refuse automatic activation when regression gates fail?
