...
- A Playwright `webServer` timeout can be caused by a stale `vite preview` process already listening on port 4173; check `ss -ltnp | grep ':4173\\b'`, `ps -fp <pid>`, and `readlink -f /proc/<pid>/cwd` before rerunning smoke tests.
- If the stale preview process cwd points to a deleted issue worktree (`readlink /proc/<pid>/cwd` shows `...(deleted)`) or the server returns 404 for `/`, it is safe to stop only that confirmed stale Vite preview as a stale task artifact before rerunning Playwright.
