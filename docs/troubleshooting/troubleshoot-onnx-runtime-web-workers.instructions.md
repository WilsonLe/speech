...
- After normalizing package/lock files back to the registry ORT spec, avoid `pnpm exec` or filtered `pnpm --filter ...` scripts in a fresh worktree unless dependencies are already reconciled; they may trigger a fresh ORT fetch. Use direct local binaries for final non-fetching checks, or relink the local tarball for validation and normalize again before committing.
