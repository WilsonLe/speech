...
def test_all_package_versions_are_v0_6_1() -> None:
    versions = {
    ...
    assert "0.6.1" in PERSONAL_MODELS_PANEL.read_text(encoding="utf-8")
...
def test_current_state_points_to_hotfix_release_without_closing_255() -> None:
    current = read_json(CURRENT_STATE)

    assert current["packageVersion"] == "0.6.1"
    assert current["currentRelease"] == "v0.6.1-csp-hotfix"
    assert "v0.6.1-csp-hotfix" in current["lineage"]
    assert (
        current["verifiedGates"]["releaseNotes"]
        == "docs/planning/v0.6.1-csp-hotfix-release-notes.json"
    )
    assert (
        current["verifiedGates"]["releasePublicationChecklist"]
        == "docs/planning/v0.6.1-release-publication-checklist.json"
    )
